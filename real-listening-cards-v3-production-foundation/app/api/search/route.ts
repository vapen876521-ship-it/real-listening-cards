...
import { detectOralVariants, normalize } from "@/lib/pronunciation";
...
export async function GET(request: Request) {
  ...
  if (!key) {
    return NextResponse.json(
      { error: "缺少 YOUTUBE_API_KEY。请在 Vercel 的 Environment Variables 里添加。" },
      { status: 500 }
    );
  }
  ...
  const samples = await Promise.all(
    items.map(async (item): Promise<PronunciationSample> => {
      ...
      const sourceDiversityKey = item.snippet.channelTitle;
      ...
      return {
        ...
        reviewEligible,
        sourceType: transcript.found ? "youtube_real_video" : "caption_unavailable",
        tags: [
          ...
          ...analysis.oralVariants.map((variant) => variant.label)
        ].filter(Boolean),
        note: reviewEligible
          ? "字幕精确匹配，片段长度合适，可进入复习。"
          : "这个样本质量不足，只作为备选来源展示。"
  ...
  const ranked = rankAndDiversify(samples);
  const highQualitySamples = ranked.filter((sample) => sample.reviewEligible && sample.qualityScore >= 70).slice(0, 8);
  const backupSamples = ranked.filter((sample) => !highQualitySamples.some((good) => good.id === sample.id)).slice(0, 8);
